// Options page — the home for configuration: tasks & goals, overrun alerts,
// idle detection, and Google Sheets push. Reads/writes the same storage keys
...
#options-toast{position:fixed;bottom:28px;left:50%;transform:translateX(-50%) translateY(20px);
...
function renderTasks() {
  ...
  state.tasks.forEach((t, i) => {
    ...
    name.addEventListener('change', async () => { state.tasks[name.dataset.i].name = name.value; await saveTasks(); });
...
$('notif-toggle').addEventListener('change', async () => {
  await setStorage(SK.notifOn, String($('notif-toggle').checked));
});
$('notif-mins').addEventListener('change', async () => {
  await setStorage(SK.notifMins, String(parseInt($('notif-mins').value) || 90));
});
$('idle-toggle').addEventListener('change', async () => {
  await setStorage(SK.idleOn, String($('idle-toggle').checked));
});
$('idle-mins').addEventListener('change', async () => {
  await setStorage(SK.idleMins, String(parseInt($('idle-mins').value) || 5));
});
...
$('sheets-url').addEventListener('change', async () => {
  ...
  await setStorage(SK.sheetsUrl, url);
