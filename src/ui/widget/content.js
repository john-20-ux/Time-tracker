...
//
// This script can be injected two ways (registered content script on enabled
// pages, or executeScript from the popup's "pop out" button), so it guards
// against mounting twice on the same page.
...
(function mountWidget() {
  if (document.getElementById('time-tracker-ext-host')) return; // already mounted
...
`;
  ...
  // The − button minimizes to the circle; clicking the circle restores.
  ...
  // React to minimize/restore triggered elsewhere (e.g. the popup's pop-out).
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== 'local' || !(SK.widgetMinimized in changes)) return;
    tracker.classList.toggle('minimized', changes[SK.widgetMinimized].newValue === 'true');
  });
